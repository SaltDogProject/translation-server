...
Zotero.HTTP = new (function () {
    ...
    this.httpMaxResponseSize =
        (config.has('httpMaxResponseSize') ? config.get('httpMaxResponseSize') : 10) * 1024 * 1024;
    ...
    this.request = async function (method, requestURL, options = {}) {
        ...
        if (['GET', 'HEAD'].includes(method)) {
            ...
            logBody =
                `: ${options.body.substr(0, options.logBodyLength)}` + options.body.length > options.logBodyLength
                    ? '...'
                    : '';
    ...
    this.doGet = function (url, onDone, responseCharset, cookieSandbox, headers) {
        ...
        this.request('GET', url, { responseCharset, headers, cookieSandbox }).then(onDone, function (e) {
    ...
    this.doPost = function (url, body, onDone, headers, responseCharset, cookieSandbox) {
        ...
        this.request('POST', url, { body, responseCharset, headers, cookieSandbox }).then(onDone, function (e) {
            onDone({ status: e.status, responseText: e.responseText });
            throw e;
        });
        return true;
        ...
        let req = request({
            ...
            .on('response', function (res) {
                ...
                if (!supported) {
                    ...
                    returned = true;
                    return reject(
                        new Zotero.HTTP.UnsupportedFormatError(
                            requestURL,
