...
const TLDS = require('../modules/translate/src/tlds');
...
WebSession.prototype.handleURL = async function () {
    ...
    if (m) {
        let domain = m[1];
        let blacklisted = config.get('blacklistedDomains').some((x) => x && new RegExp(x).test(domain));
        if (blacklisted) {
    ...
    for (let i = 0; i < urlsToTry.length; i++) {
        ...
        translate.setHandler(
            'translators',
            async function (translate, translators) {
        ...
        translate.setHandler('select', (translate, items, callback) => {
            try {
                this.select(url, translate, items, callback, translatePromise);
            } catch (e) {
                Zotero.debug(e, 1);
                reject(e);
