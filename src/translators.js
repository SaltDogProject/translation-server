...
Translators = Object.assign(
    ...
    new (function () {
        ...
        this.init = async function (translators) {
            if (!translators) {
                translators = global._translators;
            }
        ...
        //     var translatorsDirPath = path.resolve(
        ...
        this.getCodeForTranslator = Zotero.Promise.method(function (translator) {
            if (translator.code) {
                ...
                if (translator.metadata.localPath) {
                    Zotero.debug('Load ' + translator.metadata.localPath);
                    let data = fs.readFileSync(translator.metadata.localPath, 'utf8');
                    if (data[0] === '\uFEFF') data = data.substr(1);
                    translator.cacheCode = 1;
                    translator.code = data;
                    return data;
                } else {
        ...
        this.getWebTranslatorsForLocation = async function (URI, rootURI) {
            ...
            for (var i = 0; i < allTranslators.length; i++) {
                ...
                rootURIsLoop: for (var rootSearchURI in rootSearchURIs) {
                    ...
                    var rootURIMatches =
                        isGeneric || (rootSearchURI.length < 8192 && translator.webRegexp.root.test(rootSearchURI));
                    if (translator.webRegexp.all && rootURIMatches) {
                        for (var frameSearchURI in frameSearchURIs) {
                            var frameURIMatches =
                                frameSearchURI.length < 8192 && translator.webRegexp.all.test(frameSearchURI);
