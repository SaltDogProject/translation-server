...
describe('/export', function () {
    var json = [
        {
            ...
            creators: [{ firstName: 'Keith', lastName: 'Collins', creatorType: 'author' }],
            tags: [
                { tag: 'Net Neutrality', type: 1 },
                { tag: 'Pai, Ajit', type: 1 },
                { tag: 'Federal Communications Commission', type: 1 },
                { tag: 'Regulation and Deregulation of Industry', type: 1 },
            ...
            accessDate: '2018-08-21T08:23:37Z',
            date: '2018-06-11',
    ...
    var jsonMinimal = [
        {
            ...
            title: 'Example',
            accessDate: '2019-01-14T20:18:11Z',
        },
    ];

    it('should export to BibTeX (full)', async function () {
        var response = await request().post('/export?format=bibtex').send(json).expect(200);
        assert.isTrue(response.text.trim().startsWith('@article'));
    });

    it('should export to BibTeX (minimal)', async function () {
        var response = await request().post('/export?format=bibtex').send(jsonMinimal).expect(200);
    ...
    it('should export to RIS (full)', async function () {
        var response = await request().post('/export?format=ris').send(json).expect(200);
        assert.isTrue(response.text.startsWith('TY  - NEWS'));
    });

    it('should export to RIS (minimal)', async function () {
        var response = await request().post('/export?format=ris').send(jsonMinimal).expect(200);
