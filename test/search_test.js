...
describe('/search', function () {
    ...
    beforeEach(() => {
        ...
        sinon.stub(HTTP, 'request').callsFake(async function (method, url, options) {
            ...
            if (url.startsWith('http://lx2.loc.gov')) {
                var xml = fs.readFileSync(path.join(__dirname, 'data', 'loc_book1_response.xml'), {
                    encoding: 'utf-8',
                });
                return {
                    status: 200,
            ...
            if (url.startsWith('https://eutils.ncbi.nlm.nih.gov')) {
                let xml = fs.readFileSync(path.join(__dirname, 'data', 'pubmed_article1_response.xml'), {
                    encoding: 'utf-8',
                });
                return {
                    status: 200,
    ...
    it('should perform a text search', async function () {
        ...
        var response = await request().post('/search').set('Content-Type', 'text/plain').send(bookTitle1.toLowerCase());
    ...
    it('should translate an ISBN', async function () {
        var response = await request().post('/search').set('Content-Type', 'text/plain').send(bookISBN1);
    ...
    it('should translate a PMID', async function () {
        var response = await request().post('/search').set('Content-Type', 'text/plain').send(articlePMID1);
        assert.equal(response.statusCode, 200);
        var json = response.body;
